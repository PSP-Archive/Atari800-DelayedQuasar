...
static const int ScreenBufferSkip = SCREEN_BUFFER_SIZE - Screen_WIDTH;

PspImage *Screen;

static int ParseInput();
static void CopyScreenBuffer();
static void AudioCallback(short* buf, unsigned int samples);
...
int InitEmulation()
{
  ...
  if (!(Screen = pspImageCreateVram(SCREEN_BUFFER_SIZE, 
  	SCREEN_BUFFER_SIZE, PSP_IMAGE_INDEXED)))
    	return 0;
  ...
  pl_snd_set_callback(AudioCallback);
...
int Sound_Initialise(int *argc, char *argv[])
{
  ...
  return POKEYSND_Init(POKEYSND_FREQ_17_EXACT, SOUND_FREQ, 1, POKEYSND_BIT16);
}
...
// Sound thin api
// Remove sound.o in Makefile, enable SOUND_THIN_API + SOUND_CALLBACK in config.h, for now it crashes
// static void AudioCallback(short* buf, unsigned int samples)
// {
//   Sound_Callback((unsigned char*)buf, 2 * samples);
// }

// int PLATFORM_SoundSetup(Sound_setup_t *setup)
// {
//   setup->freq = SOUND_FREQ;
//   setup->sample_size = 2;
// 	setup->channels = 1;
// 	setup->buffer_frames = SND_FRAG_SIZE;

//   // PSP is too slow for new pokey
//   POKEYSND_enable_new_pokey = 0;
...
void RunEmulation()
{
  ...
  case DISPLAY_MODE_FIT_HEIGHT:
    ...
    ScreenW = (int)((float)Screen->Viewport.Width * ratio + 0.5f);
    ScreenH = SCR_HEIGHT;
    break;
  ...
  while (!ExitPSP)
  {
    ...
    if (++Frame > Config.Frameskip && Atari800_display_screen)
    {
