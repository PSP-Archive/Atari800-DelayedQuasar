...
*/
  {
    ...
    short start, end;
    short sc_end, slsz_scaled;
    short sxr, dxr;
    slsz_scaled = (short)ceil((float)dw * (float)SLICE_SIZE) / (float)image->Viewport.Width;
    ...
    for (; start < end; start += SLICE_SIZE, dx += slsz_scaled)
    {
      ...
      vertices[1].u = (sxr > end) ? end : sxr;
      vertices[1].v = image->Viewport.Height + image->Viewport.Y;

      vertices[0].x = dx; 
      vertices[0].y = dy;
      vertices[1].x = (dxr > sc_end) ? sc_end : dxr; 
      vertices[1].y = dy + dh;
