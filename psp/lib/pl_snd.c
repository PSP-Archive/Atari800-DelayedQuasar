...
int pl_snd_init(int sample_count)
{
  ...
  /* Initialize buffers */
  sound_stream.sample_buffer[0] = memalign(64, 2 * sample_count * sizeof(short)); // Alloc for both buffers!
  if (!sound_stream.sample_buffer[0])
  {
    return 0;
  }
  memset(sound_stream.sample_buffer[0], 0, 2 * sample_count * sizeof(short));	// clean the buffer 
...
static int channel_thread(int args, void *argp)
{
  ...
  while (!sound_stop)
  {
    ...
    {
      memset(bufptr, 0, sound_stream.samples[bufidx] * sizeof(short));
    }
