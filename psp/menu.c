...
static void SetMachineTypeInternal(int type)
{
  Atari800_builtin_basic = TRUE;
  Atari800_SetMachineType(type);
}
...
PL_MENU_OPTIONS_BEGIN(MachineTypeOptions)
  PL_MENU_OPTION("Atari 400/800 (48 KB)",MACHINE_TYPE(Atari800_MACHINE_800,48))
...
PL_MENU_ITEMS_BEGIN(SystemMenuDef)
  ...
  PL_MENU_ITEM("Emulate high frequencies", SYSTEM_EMUL_HIGH_FR, ToggleOptions, 
    "\026\250\020 Toggle high frequencies emulation")
  PL_MENU_HEADER("Video")
...
void InitMenu()
{
  ...
  LOG_DEBUG("InitMenu have roms:%d\n", have_roms);
...
void DisplayMenu()
{
  ...
  do
  {
    ...
    /* Reset viewport */
    Screen->Viewport.Width = ATARI_VISIBLE_WIDTH;
    Screen->Viewport.X = (Screen_WIDTH-Screen->Viewport.Width)/2;
    ...
    case TAB_SYSTEM:
      item = pl_menu_find_item_by_id(&SystemUiMenu.Menu, SYSTEM_EMUL_HIGH_FR);
      pl_menu_select_option_by_value(item, (void*)POKEYSND_bienias_fix);
...
void LoadOptions()
{
  ...
  int machine_type = pl_ini_get_int(&init, "System", "Machine Type", Atari800_MACHINE_XLXE);
  SetMachineTypeInternal(machine_type);
  ...
  POKEYSND_bienias_fix = pl_ini_get_int(&init, "System", "Emul High Freq", 0);
...
static int SaveOptions()
{
  ...
  pl_ini_set_int(&init, "System", "Emul High Freq", POKEYSND_bienias_fix);
                             ...
                             const pl_menu_option* option)
{
  ...
  else if (uimenu == &SystemUiMenu)
  {
    ...
    switch(item->id)
    {
    case SYSTEM_EMUL_HIGH_FR:
      POKEYSND_bienias_fix = (int)option->value;
      break;
    ...
    case SYSTEM_MACHINE_TYPE:
      ...
      SetMachineTypeInternal(MACHINE((int)option->value));
...
static int OnSwitchDiskImageOk(const void *browser, const void *file_path)
{
  ...
  {
    reset = 1;
    SetMachineTypeInternal(Atari800_MACHINE_XLXE);
    MEMORY_ram_size = 64;
    Atari800_InitialiseMachine();
  }
  ...
  if (!AFILE_OpenFile((char*)path, reset, 1, 0))
  {
    if (reset)
    {
      SetMachineTypeInternal(old_mach);
...
int LoadCartridge(const char *path)
{
  ...
  default:
	...
	{
    CARTRIDGE_SetType(&CARTRIDGE_main, cart_type);
	}
...
{
  LOG_DEBUG("OnQuickloadOk machine:%d, ram:%d\n", Atari800_machine_type, MEMORY_ram_size);
  /* Eject disk and cartridge */
  CARTRIDGE_Remove();
  SIO_Dismount(1);

  const char *path = PrepareFile(file_path);
  LOG_DEBUG("OnQuickloadOk prepare file path:%s\n", path);

  int fileType = AFILE_DetectFileType(path);
  LOG_DEBUG("OnQuickloadOk detect file type:%d\n", fileType);
  switch (fileType)
  {
	case AFILE_CART:
	case AFILE_ROM:
  ...
  /* Reset current game path */
  pl_file_get_parent_directory(LoadedGame, GamePath, sizeof(GamePath));
  LOG_DEBUG("OnQuickloadOk game:%s, dir:%s\n", LoadedGame, GamePath);
